...
function _M:init(title, actor, filter, action)
	...
	self.filter = filter
	self.action = action
	Dialog.init(self, title or "Equipment", math.max(800, game.w * 0.8), math.max(600, game.h * 0.8), nil, nil, nil, gameFont)

	self.c_desc = TextzoneList.new{width=math.floor(self.iw / 2 - 10), height=self.ih, no_color_bleed=true}

	self.c_list = ListColumns.new{font=basic,width=math.floor(self.iw / 2 - 10), height=self.ih - 10, scrollbar=true, columns={
