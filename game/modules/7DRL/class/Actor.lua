	...
	table.sort(invens, function(a,b) if a.id == 1 then return false elseif b.id == 1 then return true else return a.id < b.id end end)
		for _, inven in ipairs(invens) do
			for i, o in ipairs(inven) do
				if o.droppable then
					local rngDrop = math.random(1,100)
					if rngDrop <= o.dropRate then
						o.droppedBy = self.name
						-- Adding object to map on selfs location.
						game.level.map:addObject(self.x, self.y, o)
						-- Displaying to the log who dropped what item.
						game.chatLog(o.droppedBy.." dropped "..o.name)
					end
				end
			end
		-- Clearing any Inventory
		self.inven = {}
	end
...
function _M:gearLeveling()
	
	game.logPlayer(self, "Checking Inventory on Level Up")
	for inven_id =  1, #self.inven_def do
		if self.inven[inven_id] and (self.inven_def[inven_id].is_worn) then
			for item, o in ipairs(self.inven[inven_id]) do
				if o.lvlup then
					self:incIncStat('str', o.lvlup.str)
					self:incIncStat('dex', o.lvlup.dex)
				end
			end
		end
	end
