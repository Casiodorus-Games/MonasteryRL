...
function getColor(item, type) -- Item and then what type of damage we're looking for
	elementColor = {}
	if type == "weapon" then
		if item.object.combat.physical_damage > 0 then table.insert(elementColor, "#LIGHT_SLATE#") end
		if item.object.combat.arcane_damage > 0 then table.insert(elementColor, "#VIOLET#") end
		if item.object.combat.holy_damage > 0 then table.insert(elementColor, "#MOCCASIN#") end
		if item.object.combat.unholy_damage > 0 then table.insert(elementColor,"#STEEL_BLUE#") end
	end
	if type == "armor" then
		if item.object.resistances.physical_resistance > 0 then table.insert(elementColor,"#LIGHT_SLATE#") end
		if item.object.resistances.arcane_resistance > 0 then table.insert(elementColor,"#VIOLET#") end
		if item.object.resistances.holy_resistance > 0 then table.insert(elementColor,"#MOCCASIN#") end
		if item.object.resistances.unholy_resistance > 0 then table.insert(elementColor,"#STEEL_BLUE#") end
	end
	return elementColor
end
