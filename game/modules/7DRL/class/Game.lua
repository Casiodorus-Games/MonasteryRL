...
-- Local Files for sizing game options
local fontsize = 10
local logFontSize = 10
local tile_x = 16
local tile_y = 16
gameFont = "/data/general/font/Ac437_IBM_CGA.ttf"

spawnKeyLoc = math.random(1,2)

-- First time in locations
firstCrypt = true
north_belfrey = true
...
function _M:run()
	...
	self.player_display = PlayerDisplay.new(0, 0, 200, self.h, {89,61,59}, gameFont, 12) 
	self.logdisplay = LogDisplay.new(self.w *0.1, self.h * 0.8, self.w * 0.5, self.h * 0.2, nil, gameFont, logFontSize, {255,255,255}, {30,30,30})
	self.chatDisplay = LogDisplay.new(self.w * 0.16, self.h * 0.8, self.w * 0.48, self.h * 0.2, nil, gameFont, logFontSize, {255,255,255}, {30,30,30})
	self.combatDisplay = LogDisplay.new(self.w * 0.645, self.h * 0.8, self.w * 0.35, self.h * 0.2, nil, gameFont, logFontSize, {255,255,255}, {30,30,30})
	...
	self.combatLog = function(style, ...) if type(style) == "number" then self.combatDisplay(...) self.flash(style, ...) else self.combatDisplay(style, ...) self.flash(self.flash.NEUTRAL, style, ...) end end
	self.combatSeen = function(e, style, ...) if e and self.level.map.seens(e.x, e.y) then self.combatLog(style, ...) end end
	self.combatLog(" ")

	self.chatLog = function(style, ...) if type(style) == "number" then self.chatDisplay(...) self.flash(style, ...) else self.chatDisplay(style, ...) self.flash(self.flash.NEUTRAL, style, ...) end end
	self.chatSeen = function(e, style, ...) if e and self.level.map.seens(e.x, e.y) then self.chatLog(style, ...) end end
	self.chatLog("The Monastery before you has been infested by the Cult of Ygothr. Hunt and eradacate the heretics.")
...
function _M:loaded()
	...
	Map:setViewerActor(self.player)
	Map:setViewPort(200, 20, self.w - 200, math.floor(self.h * 0.80) - 20, tile_x, tile_y, gameFont, fontsize, true)
...
function _M:changeLevel(lev, zone)
	...
	if self.zone.wilderness then
		...
		game.log("Back at the monastery.")
		self.player:move(x,y, true)
	end

	if self.zone.name == "The Crypt" and firstCrypt == true then
		game.chatLog("You have entered the crypt and the smell of decay is strong here.")
		firstCrypt = false
	end
end
...
function _M:display(nb_keyframe)
	...
	if self.level and self.level.map and self.level.map.finished then
		...
		-- And the minimap
		self.level.map:minimapDisplay(0, 0, util.bound(self.player.x - 25, 0, self.level.map.w - 50), util.bound(self.player.y - 25, 0, self.level.map.h - 50), 50, 50, 0.6)
	end
	...
	self.combatDisplay:toScreen()
	self.chatDisplay:toScreen()
